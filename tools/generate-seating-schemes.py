...
# The page carrying the inline names-variant plans (between the markers below);
# main() re-injects them on every run so the page never drifts from this source.
PAGE = ROOT / "site" / "practical-info.html"
MARK_BEGIN = ("<!-- seating-plans:begin "
              "(generated by tools/generate-seating-schemes.py - do not edit between markers) -->")
MARK_END = "<!-- seating-plans:end -->"
...
def inline_svg(svg: str, sfx: str, label: str) -> str:
    """Adapt an emitted SVG document for inlining into the page: the page owns
    the fonts (drop the @import <style>), defs ids get a per-plan suffix so the
    two inlined plans don't collide, and the root gains class + a11y naming.
    width/height stay - with the site's global svg rule they cap at native size
    and shrink responsively."""
    svg = re.sub(r"<style>.*?</style>", "", svg, count=1, flags=re.S)
    for ident in ("hatch", "arrow"):
        svg = svg.replace(f'id="{ident}"', f'id="{ident}-{sfx}"')
        svg = svg.replace(f"url(#{ident})", f"url(#{ident}-{sfx})")
    return svg.replace(
        "<svg ",
        f'<svg class="seating-plan" role="img" aria-label="{label}" ',
        1,
    )


def inject_into_page() -> None:
    """Replace the marker block in the page with fresh inline names-variant plans."""
    html_text = PAGE.read_text(encoding="utf-8")
    if MARK_BEGIN not in html_text or MARK_END not in html_text:
        print(f"note: seating markers not found in {PAGE.relative_to(ROOT)} - page not touched")
        return
    m = MODES["names"]
    block = "\n      ".join([
        MARK_BEGIN,
        inline_svg(build_inside(m), "in", "Zasedací pořádek uvnitř restaurace"),
        inline_svg(build_outside(m), "out", "Zasedací pořádek venku na terase"),
        MARK_END,
    ])
    before = html_text.split(MARK_BEGIN)[0]
    after = html_text.split(MARK_END, 1)[1]
    PAGE.write_text(before + block + after, encoding="utf-8")
    print(f"injected inline plans into {PAGE.relative_to(ROOT)}")
