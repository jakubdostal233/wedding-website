...
The current visual identity is the palette ivory / charcoal / sage green with Cormorant Garamond (display serif) and Inter (body sans), loaded from the Google Fonts CDN. This identity is scheduled to be refreshed to match the printed wedding materials &mdash; a blush/dusty-pink palette &mdash; in a dedicated phase; see [O-DESIGN](../dev/decisions.md#o-design--visual-design-refresh-from-the-print-materials). Because the palette and type live as custom properties in `main.css`, that refresh is largely a change to the token block rather than a per-page edit.
