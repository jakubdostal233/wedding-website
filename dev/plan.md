...
2. 🪑 **Embed the seating schemes** (`site/assets/img/seating-{inside,outside}.svg`, generated by `tools/generate-seating-schemes.py`). Owner decisions first, then a SPEC update before touching pages: (a) which page/section (program? praktické informace? new unlisted page?); (b) inline `<svg>` vs `<img>` &mdash; `<img>` blocks the SVG's Google-Fonts import, inline uses the page's fonts; (c) the planned names-on-seats round &mdash; larger seat chips, and a conscious yes/no on guest names being served publicly.
