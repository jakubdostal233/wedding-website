...
| `practical-info.html` | Praktické informace | Dress code, doprava a parkování (incl. Gröbovka pro focení), menus, zasedací pořádek (inline seating plans with guest names), program pro děti, Dar (thank-you), Různé, Kontakt |
| `photoshooting.html`  | Focení              | Photo-shoot groups (organises the shoot)                                                                                                                                        |
| `gift.html`           | (Dar)               | Bank QR + IBAN &mdash; **not in the nav, no on-site links**; reachable only directly at `/gift`                                                                                 |

Header nav is **Program · Praktické informace · Focení** (the brand links home to `index`). `gift.html` is kept out of the nav **and, since 2026-07-07, off every page** &mdash; the Dar section thanks without linking, and the page is reachable only via the direct `/gift` URL shared personally. See [D-IA4](./decisions.md#d-ia4--site-reduced-to-four-pages).
